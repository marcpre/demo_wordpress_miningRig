...
'+'-es
 <ul>
     <li>
         <synonym words="{{$model}}|{{$modelWithoutManufacturer}}">The {{$model}}</synonym> <synonym words="is not that expensive">is quite affordable</synonym>
     </li>
     <li>
         <synonym words="Watt usage is low, wattage is: {{ $powerConsumption }}, Watt usage of {{ $powerConsumption }}"> Low watt usage of:&nbsp; </synonym> <insertdata>{{ $powerConsumption }}</insertdata>
     </li>
     <li>With the <synonym words="algorithm">current algorithm</synonym>,&nbsp;<insertdata> - {{$algorithm}} -,</insertdata>&nbsp;<synonym words="numerous| |various">multiple</synonym>&nbsp;<synonym words="cryptos| coins">cryptocurrencies</synonym> can be mined
     </li>
     <li>
...
'-'-es
 <ul>
     ...
     <li>
         <synonym words="Profitability is very low with the {{$modelWithoutManufacturer}}">Mining is not profitable with the<synonym words="{{$modelWithoutManufacturer}}">{{$model}}</synonym></synonym><synonym words="Cryptocurrency Mining| Crypto mining"></synonym>
     </li>
 </ul>
...
<h2><synonym words="Expenses| Electricity Costs| Operating Costs| Operating Expenses| Costs for Electricity| Costs for Operating the Rig">Costs</synonym>
    <b></b></h2>
In our<synonym words=" | ">profitability</synonym> calculations we assume an overall <synonym words="costs | charge | amount | expenditure">price</synonym> of 0.1. Hence we derive a cost baseline of&nbsp;<insertdata>{{ $miningCosts }}</insertdata>&nbsp;. Hence the miner has&nbsp;
<insertdata>{{$dailyProfitOfMiner}}</insertdata>&nbsp;until the miner gets profitable. (as per <insertdata>{{$monthToday}}</insertdata>).
